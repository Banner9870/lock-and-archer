...
import { getCommunityAreaById } from "@/lib/community-areas";
...
export default async function CommunityFeedPage({ params }: PageProps) {
  ...
  const decodedId = decodeURIComponent(communityId?.trim() ?? "");
  const area = getCommunityAreaById(decodedId);

  const items = await getUnifiedFeedItems(30, decodedId);
  const areaName = area?.name ?? decodedId;
  ...
  return (
    <>
      ...
      >
        Guides (and articles when tagged) in this community area
      </p>
      ...
      {items.length === 0 ? (
        <p
          ...
          style={{ color: "var(--text-secondary)" }}
        >
          No guides or articles in this area yet.
        </p>
      ) : (
